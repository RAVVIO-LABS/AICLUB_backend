        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-body">
                    @if ($teacher->kyc_data)
                        <ul class="list-group">
                            @foreach ($teacher->kyc_data as $val)
                                @continue(!$val->value)
                    ...
                    @if ($teacher->kv == Status::KYC_UNVERIFIED)
                        <div class="my-3">
                            <h6>@lang('Rejection Reason')</h6>
                            <p>{{ $teacher->kyc_rejection_reason }}</p>
                        </div>
                    @endif

                    @if ($teacher->kv == Status::KYC_PENDING)
                        <div class="d-flex flex-wrap justify-content-end mt-3">
                            ...
                            <button class="btn btn-outline--success confirmationBtn" data-question="@lang('Are you sure to approve this documents?')"
                                data-action="{{ route('admin.teachers.kyc.approve', $teacher->id) }}"><i
    ...
    <div id="kycRejectionModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.teachers.kyc.reject', $teacher->id) }}" method="POST">
                    @csrf
                    <div class="modal-body">
                        <div class="alert alert-primary p-3">
                            @lang('If you reject these documents, the teacher will be able to re-submit new documents and these documents will be replaced by new documents.')
                        </div>
