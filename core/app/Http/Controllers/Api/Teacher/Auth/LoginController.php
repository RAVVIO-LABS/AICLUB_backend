...
class LoginController extends Controller
{
    ...
    */
    ...
    public function login(Request $request)
    {
        ...
        if (!Auth::guard('teacher')->attempt(array_merge($credentials, ['role_type' => 'teacher']))) {
        ...
        if ((int) $user->status !== Status::USER_ACTIVE) {
            ...
            $isPendingApproval = empty($user->ban_reason);
            $notify[] = $isPendingApproval ? 'Your account is pending admin approval' : 'Your account is banned';
            return responseError($isPendingApproval ? 'teacher_pending_approval' : 'banned', $notify, [
                'user' => $user
            ]);
        }
      
        $tokenResult = $user->createToken('auth_token', ['teacher'])->plainTextToken;
    ...
    public function socialLogin(Request $request)
    {
        ...
        $socialLogin = new SocialLogin($provider, true,'teacher');
        try {
